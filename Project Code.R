...
lmcs1 <- lm(formula = as.numeric(Satisfaction)~ Age, data = Finalcheapseats)
summary(lmcs1)
lmcs1
lmcs2 <- lm(formula = as.numeric(Satisfaction)~ Gender, data = Finalcheapseats)
summary(lmcs2)
lmcs2
lmcs3 <- lm(formula = as.numeric(Satisfaction)~ Price.Sensitivity, data = Finalcheapseats)
summary(lmcs3)
lmcs3
lmcs4 <- lm(formula = as.numeric(Satisfaction)~Eating.and.Drinking.at.Airport , data = Finalcheapseats)
summary(lmcs4)
lmcs4 # not significant
lmcs5 <- lm(formula = as.numeric(Satisfaction)~Flight.cancelled , data = Finalcheapseats)
summary(lmcs5)
lmcs5
lmcs6 <- lm(formula = as.numeric(Satisfaction)~Arrival.Delay.greater.5.Mins , data = Finalcheapseats)
summary(lmcs6)
lmcs6
lmcs7 <- lm(formula = as.numeric(Satisfaction)~Airline.Status , data = Finalcheapseats)
summary(lmcs7)
lmcs7
lmcs8 <- lm(formula = as.numeric(Satisfaction)~ Class , data = Finalcheapseats)
summary(lmcs8)
lmcs8
lmcs9 <- lm(formula = as.numeric(Satisfaction)~ Type.of.Travel , data = Finalcheapseats)
summary(lmcs9)
Lmcs9
...
#for ggmaps
statesCovered<-ddply(dfAir1,~Airline.Name,mean(Arrival.Delay.in.Minutes),States=length(unique(Destination.State)))
View(dfAir1)
summary(dfAir1)


#Removing 9999 Values which were introduced for removing Na's
sqldf(' select count("Arrival.Delay.in.Minutes") from dfAirif where "Arrival.Delay.in.Minutes" = 9999')
...
summary(lmcs4)
lmcs4 # not significant
# lmcs5 <- lm(formula= as.numeric(Satisfaction)~Flight.cancelled , data = dfAir1)
# summary(lmcs5)
# lmcs5
